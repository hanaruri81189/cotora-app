...
import { GenerationParams, SNSType, Candidate } from '../types'; // Adjust path based on your project structure
import { GEMINI_MODEL_NAME } from '../constants'; // Adjust path

// Duplicating or importing getPlatformSpecificInstructions logic
// For simplicity, defined here. Consider moving to a shared util if it grows.
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Basic validation for required fields from params
    if (!params || typeof params.episode !== 'string' || typeof params.feelings !== 'string' || !params.tone || !params.snsType) {
        return res.status(400).json({ error: "必須パラメータ（episode, feelings, tone, snsType）が不足しているか、型が正しくありません。" });
    }
...
`;
    ...
    // We cast to our custom Candidate type which might only use groundingMetadata or be simpler.
    const candidates = response.candidates as Candidate[] | undefined;
    return res.status(200).json({ text: response.text, candidates: candidates });
    ...
    if (error instanceof Error) {
      ...
      return res.status(500).json({ error: `コンテンツ生成中にエラーが発生しました: ${error.message}` });
    }
    return res.status(500).json({ error: "コンテンツ生成中に不明なサーバーエラーが発生しました。" });
  }
}